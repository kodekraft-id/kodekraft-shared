...
import { writableColumns, writersOf } from "./ownership.js";
const DEFAULT_GUARD_MODE = "throw";
...
function describeViolation(details) {
...
const UPDATE_PATTERN = new RegExp(`^\\s*update\\s+${IDENTIFIER_SOURCE}\\s+set\\s+([\\s\\S]*?)(?:\\swhere\\b|\\sreturning\\b|$)`, "i");
...
const ON_CONFLICT_UPDATE_PATTERN = new RegExp(`on\\s+conflict[\\s\\S]*?do\\s+update\\s+set\\s+([\\s\\S]*?)(?:\\swhere\\b|\\sreturning\\b|$)`, "i");
...
function identifierFromMatch(match, groupOffset) {
    return firstDefined(match[groupOffset], match[groupOffset + 1], match[groupOffset + 2], match[groupOffset + 3]);
}
function stripIdentifierQuotes(raw) {
...
function parseInsertColumnList(rawColumnList) {
...
function logViolation(error, mode) {
 ...
 */
export function enforceViolation(details, mode = DEFAULT_GUARD_MODE) {
