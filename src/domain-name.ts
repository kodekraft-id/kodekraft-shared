...
export const RESERVED_LABEL_LIST: readonly string[] = Object.freeze([...RESERVED_LABELS]);
...
function invalid(reason: string): DomainNameResult {
 ...
 */
export function normalizeDomainName(raw: string | null | undefined): DomainNameResult {
  if (typeof raw !== "string") return invalid("Nama domain wajib diisi");
  const trimmedRaw = raw.trim();
  if (trimmedRaw === "") return invalid("Nama domain wajib diisi");
  if (trimmedRaw.length > MAX_RAW_INPUT_LENGTH) return invalid("Nama domain terlalu panjang");
